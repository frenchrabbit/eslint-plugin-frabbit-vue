...
module.exports = {
  ...
  /**
   * Run callback on this destructure properties nodes.
   */
  executeOnThisExpressionDestructure(func) {
    return {
      'VariableDeclarator[init.type="ThisExpression"][id.type="ObjectPattern"][id.properties]'(
        node
      ) {
  ...
  executeOnBefoureRouteEnterInstanceProperty(func) {
    ...
    return {
      'Property[key.name=beforeRouteEnter] CallExpression[callee.name=next][arguments]'(
        node
      ) {
        if (
  ...
  executeOnWatchStringMethod(func) {
    return {
      'Property[key.name=watch] ObjectExpression[properties] Literal[value]'(
        node
      ) {
        func(node)
      },
  ...
  reportUnusedProperties(context, properties, disabledLines) {
    ...
    properties.forEach((property) => {
      if (
        disabledLines &&
        disabledLines.includes(property.node.loc.start.line)
      ) {
        return
      }

      let message = `Unused ${PROPERTY_LABEL[property.groupName]} found: "${
        property.name
      }"`
      ...
      if (context.options[0]) {
        switch (property.node.type) {
          case 'Literal':
            if (context.options[0] === 'comment') {
              const tokenAfter = context.getTokenAfter(property.node)
              fix = (fixer) => [
                fixer.insertTextBefore(property.node, '/*'),
          ...
          case 'Identifier':
            if (context.options[0] === 'comment') {
              const tokenAfter = context.getTokenAfter(property.node.parent)
              fix = (fixer) => [
                fixer.insertTextBefore(property.node.parent, '/*'),
                fixer.insertTextAfter(
                  tokenAfter.value === ',' ? tokenAfter : property.node.parent,
                  '*/'
                ),
              ]
            }
            break
          default:
            // console.log(property.node.parent)
            // fix = (fixer) => [
            //   fixer.insertTextBefore(property.node, property.node.type),
            //   fixer.insertTextBefore(property.node, '/*'),
            //   fixer.insertTextAfter(property.node, '*/'),
            // ]
            break
        }
        if (fix) {
          ...
          case 'Property':
            if (context.options[0] === 'comment') {
              ...
              fix = (fixer) => [
                fixer.insertTextBefore(property.node, '/*'),
                fixer.insertTextAfter(
                  tokenAfter.value === ',' ? tokenAfter : property.node,
                  '*/'
                ),
              ]
            }
  ...
  reportUseOfUndefinedTranslation(context, definitions, uses) {
    ...
    const badAttempts = uses.filter(
      (prop) => !definedStrings.includes(prop.value)
    )
