/**
 * @fileoverview forces to use async component import, when component have v-if or placed inside client-only/lazy-hydrate `() => import('component')`
    ...
    const templateVisitor = {
      VElement(node) {
        ...
        if (
          ...
          ['client-only', 'lazy-hydrate'].includes(
            casing.kebabCase(node.parent.rawName)
          )
        ) {
