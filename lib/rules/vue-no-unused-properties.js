 ...
 * Froked from https://github.com/learningequality/kolibri tools
 */
...
const { GROUP_PROPS, GROUP_DATA, GROUP_ASYNCDATA, GROUP_COMPUTED } = constants
...
const create = (context) => {
  ...
  const scriptVisitor = Object.assign(
    ...
    utils.executeOnThisExpressionDestructure((property) => {
      thisExpressionsVariablesNames.push(property.key.name)
    }),
    ...
    eslintPluginVueUtils.executeOnVue(context, (obj) => {
      unusedProperties = Array.from(
        eslintPluginVueUtils.iterateProperties(
          obj,
          new Set([GROUP_PROPS, GROUP_DATA, GROUP_ASYNCDATA, GROUP_COMPUTED])
        )
      )
      // console.log(obj.name, unusedProperties.map(prop => prop.name))
  ...
  return Object.assign(
    {},
    initialize,
    eslintPluginVueUtils.defineTemplateBodyVisitor(
      context,
      templateVisitor,
      scriptVisitor
    )
